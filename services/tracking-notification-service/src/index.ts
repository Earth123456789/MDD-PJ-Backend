// tracking-notification-service/src/index.ts
...
app.get('/health', (req, res) => {
  res.status(200).json({ status: 'ok', service: 'tracking-notification-service' });
});

// Start server
const server = app.listen(port, async () => {
  logger.info(`Tracking & Notification Service running on port ${port}`);
