...
async def handle_driver_websocket(
    ...
    This function:
    1. Validates the driver's JWT token
    ...
    try:
        # Verify the token and extract driver ID
        token_data = decode_jwt(driver_token)
        driver_id = token_data.get("sub")
        
        if not driver_id or not token_data.get("is_driver", False):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, 
                detail="Invalid driver token"
            )
        
        # Convert driver_id to int
...
async def handle_user_websocket(
    ...
    try:
        ...
        token_data = decode_jwt(user_token)
        ...
        while True:
            ...
            try:
                ...
                if message_type == "subscribe_order":
                    ...
                    if order_id:
                        order = await order_service.get_order_tracking(int(order_id))
                        if order and order.user_id == user_id:
                            await connection_manager.subscribe_to_order(connection_id, int(order_id))
                            await websocket.send_json({
                                "type": "subscribed",
                                "data": {
                                    "order_id": order_id,
                    ...
                    order_id = message_data.get("data", {}).get("order_id")
                    if order_id:
                        await connection_manager.unsubscribe_from_order(connection_id, int(order_id))
                        await websocket.send_json({
                            "type": "unsubscribed",
                            "data": {
                                "order_id": order_id
