...
// เปลี่ยน Type ของ user_id จาก number เป็น string
interface UserCacheData {
  user_id: string;           // เปลี่ยนจาก number เป็น string
  email?: string;
  full_name?: string;
  phone?: string;
  role?: string;
...
export class UserCacheService {
  ...
  public async cacheUserData(data: UserCacheData): Promise<any> {
    try {
      ...
      if (!data.user_id) {
        throw new Error('user_id is required for caching user data');
      }
      
      logger.debug('Attempting to cache user data', { userId: data.user_id });
   ...
   */
  public async updateCachedUserData(userId: string, data: Partial<UserCacheData>): Promise<any> {
    // เปลี่ยน parameter userId จาก number เป็น string
    try {
      // ตรวจสอบว่ามี userId หรือไม่
      if (!userId) {
        throw new Error('userId is required for updating cached user data');
      }
  ...
  public async getUserData(userId: string): Promise<any> {
    ...
    try {
      // ตรวจสอบว่ามี userId หรือไม่
      if (!userId) {
        logger.error('userId is required for getting user data');
        return null;
      }
   ...
   */
  public async deleteUserCache(userId: string): Promise<boolean> {
    // เปลี่ยน parameter userId จาก number เป็น string
    try {
      // ตรวจสอบว่ามี userId หรือไม่
      if (!userId) {
        throw new Error('userId is required for deleting user cache');
      }
