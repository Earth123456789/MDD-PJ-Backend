// order-matching-service/src/index.ts
...
app.get('/health', (req, res) => {
  res.status(200).json({ status: 'ok', service: 'order-matching-service' });
});

// Start server
const server = app.listen(port, async () => {
  logger.info(`Order & Matching Service running on port ${port}`);
